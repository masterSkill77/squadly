<script setup>
import { sportLabels } from '@/Utils/sportLabels';
import TeamRow from './TeamRow.vue';

defineProps({ section: Object });
defineEmits(['add-team', 'edit-team', 'delete-team', 'delete-section']);
</script>

<template>
    <div class="rounded-xl border border-gray-100 bg-white">
        <div class="flex items-center justify-between border-b border-gray-50 px-5 py-4">
            <h4 class="text-sm font-semibold text-gray-900">{{ sportLabels[section.sport_type] || section.sport_type }}</h4>
            <div class="flex items-center gap-2">
                <span class="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-500">{{ section.teams.length }} équipe{{ section.teams.length > 1 ? 's' : '' }}</span>
                <button class="rounded p-1 text-gray-400 transition hover:bg-red-50 hover:text-red-500" @click="$emit('delete-section', section)">
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>
                </button>
            </div>
        </div>
        <div class="space-y-2 p-4">
            <TeamRow
                v-for="team in section.teams"
                :key="team.id"
                :team="team"
                @edit="$emit('edit-team', $event)"
                @delete="$emit('delete-team', $event)"
            />
            <p v-if="!section.teams.length" class="text-center text-sm text-gray-400 py-2">Aucune équipe</p>
        </div>
        <div class="border-t border-gray-50 px-5 py-3">
            <button class="inline-flex items-center gap-1 text-sm font-medium text-emerald-600 hover:text-emerald-700" @click="$emit('add-team', section)">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
                Ajouter une équipe
            </button>
        </div>
    </div>
</template>
