...
class CoachEventController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        // Competition games for this coach's club
        $club = $request->user()->resolveClub();
        $competitionGames = collect();
        if ($club) {
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $coachTeamIds = $request->user()->teams()->pluck('teams.id');
    ...
    {
        abort_if(!$request->user()->teams()->where('teams.id', $event->team_id)->exists(), 403);
